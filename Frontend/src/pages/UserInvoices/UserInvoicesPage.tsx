...
import type { UserInvoice } from "../../types/user";
import { userInvoiceApi } from "../../services/userApi";
import Badge from "../../components/Badge";
import ConfirmDeleteModal from "../../components/Modals/ConfirmDeleteModal";
import UserInvoiceFormModal from "./UserInvoiceFormModal";
...
export default function UserInvoicesPage() {
  ...
  const fetchInvoices = useCallback(async () => {
    setLoading(true);
    try {
      const data = await userInvoiceApi.getAll();
      setInvoices(data);
    } catch (err) {
      console.error("Fetch user invoices failed:", err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleDelete = async () => {
    ...
    try {
      await userInvoiceApi.remove(deleteTarget.id);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[600px]">
            <thead>
              <tr>{["Invoice #", "Customer", "Date", "Amount", "Status", "Actions"].map((h) => <TH key={h}>{h}</TH>)}</tr>
            </thead>
      ...
      {showAdd      && <UserInvoiceFormModal onClose={() => setShowAdd(false)} onSuccess={fetchInvoices} />}
      {editTarget   && <UserInvoiceFormModal initial={editTarget} onClose={() => setEditTarget(null)} onSuccess={fetchInvoices} />}
      {deleteTarget && (
        <ConfirmDeleteModal
          label={`${deleteTarget.id} – ${deleteTarget.customer}`}
